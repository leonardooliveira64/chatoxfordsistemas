...
const useStyles = makeStyles((theme) => ({
  root: {
    ...
    '& .MuiButton-outlinedPrimary': {
      color: theme.mode === 'light' ? '#3498db' : '#FFF',
        ...
        <Toolbar variant="dense" className={classes.toolbar}>
          ...
          >
            ...
            {greaterThenSm && user?.profile === "admin" && user?.company?.dueDate ? (
              <>
               Olá <b>{user.name}</b>, Bem-vindo(a) a Plataforma de Multi-Atendimento da Oxford Sistemas | <b>{user?.company?.name}</b> - (Licença Ativa até {dateToClient(user?.company?.dueDate)}).
              </>
            ) : (
              <>
		Olá <b>{user.name}</b>, Bem-vindo(a) a Plataforma de Multi-Atendimento da Oxford Sistemas | <b>{user?.company?.name}</b> - (Licença Ativa até {dateToClient(user?.company?.dueDate)}).
